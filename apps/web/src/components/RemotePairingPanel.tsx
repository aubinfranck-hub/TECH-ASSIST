...
export function RemotePairingPanel({ sessionId, sessionCode, alreadyPaired, onPaired }: Props) {
  ...
  function downloadAndRun() {
    ...
    const lines = [
      ...
      '$lines = @("rendezvous_server = ''" + $idServer + ":21116''","nat_type = 1","serial = 0","","[options]","custom-rendezvous-server = ''" + $idServer + "''","relay-server = ''" + $relayServer + "''","key = ''" + $key + "''","approve-mode = ''password-click''","verification-method = ''use-permanent-password''","allow-only-conn-window-open = ''Y''")',
